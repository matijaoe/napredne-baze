...
SELECT *
FROM klijent
WHERE imeKlijent LIKE 'mat%';

/*Primjer naredbe koja će aktivirati okidač:*/
INSERT INTO klijent (sifKlijent, imeKlijent)
VALUES (12, 'Ivan');
INSERT INTO klijent (sifKlijent, imeKlijent)
VALUES (13, 'Marko');
SELECT *
FROM klijent
WHERE sifKlijent IN (12, 13);
...
CREATE TRIGGER provjeraImenaNaUpdate
    BEFORE UPDATE
    ON klijent
    FOR EACH ROW
BEGIN
...
SELECT *
FROM klijent
WHERE prezimeKlijent = 'Bačić';
UPDATE klijent
SET imeKlijent = 'Jan'
WHERE imeKlijent = 'Nikola'
  AND prezimeKlijent = 'Bačić';
SELECT *
FROM klijent
WHERE prezimeKlijent = 'Bačić';
...
DELETE
FROM mjesto
WHERE pbrMjesto = 10000;
SELECT *
FROM klijent
WHERE pbrKlijent = 10000;
...
ALTER TABLE nastavnici
    ADD COLUMN lozinkaTimestamp TIMESTAMP DEFAULT 0;

DELIMITER %%
DROP TRIGGER IF EXISTS lozinkaZig %%
CREATE TRIGGER lozinkaZig
    BEFORE UPDATE
    ON nastavnici
    FOR EACH ROW
BEGIN
...
CREATE TRIGGER provjeriDatumUpisa
    BEFORE INSERT
    ON studenti
    FOR EACH ROW
BEGIN
...
VALUES ('1234567891', 'Ivan', 'Horvat', '2014-01-01', 10000, 10000, 1);
...
BEGIN
    ...
    INTO pozitivni
    FROM ocjene
    WHERE idKolegij = kolegij
      AND ocjena > 1;
    UPDATE kolegiji
    SET brPolozenih=pozitivni
...
CREATE TRIGGER osvjeziBrojPolozenih
    AFTER INSERT
    ON ocjene
    FOR EACH ROW
BEGIN
...
INSERT INTO ocjene
VALUES (1, '0016199452', CURDATE(), CURTIME(), 5);


/*U bazi studenti, napravite tablicu koja će služiti za BACKUP ocjena, te kreirajte TRIGGER koji će poslije svakog unosa
  ocjene kopirati tu ocjenu u BACKUP tablicu (sa svim atributima). BACKUP tablica mora dodatno sadržavati LOG podatke -
  kolonu sa imenom korisnika te kolonu sa datumom i vremenom izvršavanja akcije. */

SELECT CURRENT_USER();
#->podatak o trenutnom korisniku
SELECT CURRENT_TIMESTAMP();
#->podatak o trenutnom datumu i vremenu
SELECT NOW();
...
BEGIN
    ...
    INSERT INTO backUpOcjena (`idKolegij`, `jmbagStudent`, `datumPolaganja`, `vrijemePolaganja`, `ocjena`, imeKorisnika,
                              datumAkcije)
...
-- moj malo debilni primjer, al radi i na sve redove nakon updatea

CREATE TABLE backUpOcjena2
(
    idKolegij        INT,
    jmbagStudent     CHAR(10),
    datumPolaganja   DATE,
...
BEGIN
    ...
    SELECT CURRENT_TIMESTAMP() INTO datum;
    SELECT CURRENT_USER() INTO korisnik;
    INSERT INTO backUpOcjena2 (`idKolegij`, `jmbagStudent`, `datumPolaganja`, `vrijemePolaganja`, `ocjena`,
                               imeKorisnika,
                               datumAkcije)
    VALUES (old.idKolegij, old.jmbagStudent, old.datumPolaganja, old.vrijemePolaganja, old.ocjena, korisnik, datum);
END //
